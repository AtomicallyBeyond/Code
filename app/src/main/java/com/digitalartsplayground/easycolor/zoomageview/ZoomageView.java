package com.digitalartsplayground.easycolor.zoomageview;
...
public class ZoomageView extends AppCompatImageView implements OnScaleGestureListener {
    ...
    public void animateScaleAndTranslationToMatrix(final Matrix targetMatrix, final int duration) {
        ...
        resetAnimator.addListener(new ZoomageView.SimpleAnimatorListener() {
        ...
        if(startValues[Matrix.MSCALE_X] != targetValues[Matrix.MSCALE_X]) {
            if(!isZoomed) {
                zoomListener.isZoomed(true);
                isZoomed = true;
            }
        } else {
            if(isZoomed) {
                zoomListener.isZoomed(false);
                isZoomed = false;
            }
        }
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if(projectedScale >= startValues[Matrix.MSCALE_X]) {
            if(!isZoomed){
                zoomListener.isZoomed(true);
                isZoomed = true;
            }
